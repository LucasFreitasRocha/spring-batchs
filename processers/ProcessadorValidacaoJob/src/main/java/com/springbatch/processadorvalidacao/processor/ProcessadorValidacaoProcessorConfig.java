...
public class ProcessadorValidacaoProcessorConfig {
	...
	public ItemProcessor<Cliente, Cliente> procesadorValidacaoProcessor() throws Exception {
		return new CompositeItemProcessorBuilder<Cliente, Cliente>()
				.delegates(beanValidatingProcessor(),emailDuplicateValidatingProcessor())
				.build();
	}
	...
	private ValidatingItemProcessor<Cliente> emailDuplicateValidatingProcessor(){
		...
		return new Validator<Cliente>() {
			...
			public void validate(Cliente cliente) throws ValidationException {
				if(emails.contains(cliente.getEmail())){
					throw  new ValidationException(String.format("O cliente %s já foi processado",
							cliente.getEmail()
							));
				}
